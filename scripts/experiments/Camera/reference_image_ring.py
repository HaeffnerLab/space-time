...
import numpy as np
from ion_ring_state_detector import ion_ring_state_detector
from labrad.units import WithUnit
from multiprocessing import Process
import cv2
import math

class reference_camera_image_ring(experiment):
    ...
    def initialize(self, cxn, context, ident):
        ...
        self.fitter = ion_ring_state_detector(int(p.ion_number))
        self.ident = ident
    ...
    def run(self, cxn, context):
        ...
        self.find_initial_configuration(image)
        
    def find_initial_configuration(self,image):
        p = self.parameters.IonsOnCamera
        radial_positions = self.fitter.find_radial_positions(image,(p.ring_center_x,p.ring_center_y),threshold)
        self.pv.set_parameter('IonsOnCamera','initial_radial_positions',radial_positions)
    ...
    def fit_and_plot(self, image):
        ...
        result, params = self.fitter.guess_parameters_and_fit(xx, yy, image)  ####FIND IONS HERE
...
if __name__ == '__main__':
    ...
    exprt = reference_camera_image_ring(cxn = cxn)
